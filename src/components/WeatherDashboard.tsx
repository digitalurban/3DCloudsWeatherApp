...
export default function WeatherDashboard({ condition, realData }: { condition: string; realData: any }) {
  
  const Card = ({ title, icon: Icon, value, sub }: { title: string, icon: any, value: string | React.ReactNode, sub?: string }) => (
    <div className="backdrop-blur-xl bg-black/60 border border-white/20 p-2 md:p-3 lg:p-4 flex flex-col text-white shadow-2xl transition-all duration-300 hover:bg-black/80 flex-[0_0_auto] md:flex-1 min-w-[90px] md:min-w-0 snap-start">
      ...
      <div className="text-xs md:text-base lg:text-xl font-serif tracking-tight truncate">{value}</div>
      {sub && <div className="mt-0.5 lg:mt-1 text-[7px] md:text-[8px] lg:text-[9px] font-sans uppercase tracking-widest opacity-60 truncate">{sub}</div>}
    </div>
  );

  // Extract from realData or fallback
  const temp = realData ? `${realData.current.temperature_2m}°C` : '--°C';
  ...
  // Precipitation (Today's forecast vs current hour)
  const precipDay = realData?.daily?.precipitation_sum?.[0];
  const precip = precipDay !== undefined ? `${precipDay} mm` : '-- mm';
  
  const wind = realData ? `${Math.round(realData.current.wind_speed_10m)} mph` : '--';
  const humidity = realData ? `${realData.current.relative_humidity_2m}%` : '--%';
  const uv = realData && realData.daily ? Math.round(realData.daily.uv_index_max[0]) : '--';
  const visibility = realData ? `${(realData.current.visibility / 1000).toFixed(1)} km` : '-- km';
  const pressure = realData ? `${realData.current.surface_pressure} hPa` : '-- hPa';
  ...
  return (
    <div className="flex flex-col gap-1 lg:gap-4 pointer-events-auto w-full max-w-full mt-auto">
      ...
      <div className="flex flex-nowrap overflow-x-auto gap-[1px] bg-white/20 p-[1px] w-full shadow-2xl scrollbar-hide snap-x scroll-smooth">
        <Card title="Temperature" icon={ThermometerSun} value={temp} sub={tempSub} />
        <Card title="Precipitation" icon={Droplets} value={precip} sub="Today's Forecast" />
        <Card title="Wind" icon={Wind} value={wind} sub="Surface" />
        <Card title="Humidity" icon={CloudIcon} value={humidity} sub="Relative" />
        <Card title="UV Index" icon={SunDim} value={uv} sub="Max Today" />
        <Card title="Visibility" icon={Eye} value={visibility} sub="Surface" />
        <Card title="Pressure" icon={Gauge} value={pressure} sub="MSL" />
